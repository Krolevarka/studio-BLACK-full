<template>
  <div 
    ref="containerRef"
    class="inline-flex items-center p-1 rounded-full bg-[#0a0a0a]/90 backdrop-blur-2xl border border-white/20 shadow-[0_8px_30px_rgba(0,0,0,0.8)] relative overflow-hidden select-none"
    role="group"
  ...
  >
    ...
    <button
      ...
      tabindex="0"
      aria-label="Режим AI"
      class="relative z-10 px-6 py-2 rounded-full text-xs md:text-[13px] font-primary tracking-[0.15em] uppercase transition-colors duration-500 min-h-[2.5rem] flex items-center justify-center cursor-pointer focus:outline-none touch-manipulation active:scale-95"
...
const updateGlider = (immediate = false) => {
  ...
  if (immediate) {
    ...
    const stretch = Math.min(Math.abs(distance) * 0.22, 22)
