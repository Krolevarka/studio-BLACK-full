<template>
  <section ref="priceRef" v-bind="$attrs" class="reveal-scope-mobile relative h-svh w-full flex flex-col items-center justify-center bg-transparent z-10 touch-none overflow-hidden">
    ...
    <div
class="w-full max-w-sm mx-auto flex flex-col h-full justify-between pointer-events-none transition-[opacity,transform] duration-1000 ease-out pt-14 sm:pt-16 pb-3 sm:pb-5 px-4 sm:px-6"
         :class="[
      ...
      <!-- Навигация: Чипы -->
      <div
class="reveal-item w-full flex overflow-x-auto no-scrollbar gap-1.5 justify-start sm:justify-center pointer-events-auto shrink-0 py-1"
           :class="{ 'is-revealed': revealed }">
        <button 
          v-for="opt in options" :key="opt.id"
          class="px-3 sm:px-4 py-1.5 sm:py-2 rounded-full border text-[0.65rem] sm:text-[0.6875rem] font-secondary uppercase tracking-widest transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] flex items-center justify-center shrink-0 min-h-[2.5rem] sm:min-h-[2.75rem]"
      ...
      <div
class="reveal-item flex-1 flex flex-col justify-center items-center w-full relative pointer-events-none my-1 sm:my-3 min-h-0"
        ...
        <transition name="fade" mode="out-in">
          <div :key="activeOption.id" class="flex flex-col items-center text-center w-full">
            ...
            <p class="font-secondary text-xs sm:text-sm text-white/60 leading-relaxed max-w-[17.5rem] mb-2.5 sm:mb-4 line-clamp-2 sm:line-clamp-none">
      ...
      <div
class="reveal-item w-full pointer-events-auto my-1 sm:my-2 shrink-0"
      ...
      <!-- Итог -->
      <div
class="reveal-item w-full pointer-events-auto shrink-0"
           :class="{ 'is-revealed': revealed }" style="--reveal-delay: 320ms">
        <div class="border-t border-white/10 pt-2.5 sm:pt-4">

          <div class="flex justify-between items-end mb-2 sm:mb-4">
